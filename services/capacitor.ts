...
/**
 * True only on a native iOS build. Used to skip our custom permission
 * onboarding UI on iOS — Apple's own system alert already carries the
 * required rationale, so showing our modal first just stacks a second
 * pop-up in front of it. Requires initCapacitor() to have run at least
 * once (Capacitor is null before that, so this safely returns false).
 */
